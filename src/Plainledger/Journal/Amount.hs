...
parseAmount sep x = do
  ...
  where parseSign :: T.Text -> m (Bool, T.Text)
        parseSign "" = throwError $ mkErrorNoPos $ ParseAmountErr $ T.unpack x
        ...
        parseCoeff "" = throwError $ mkErrorNoPos $ ParseAmountErr $ T.unpack x
        parseCoeff y = 
        ...
        parseExponent :: T.Text -> m (T.Text, T.Text)
        parseExponent "" = return ("","")
        parseExponent y | T.head y == 'e' || T.head y == 'E' =
           let n = T.takeWhile isDigit (T.drop 1 y)
               r = T.drop (T.length n) y
           in return (n, r)
        parseExponent _ = throwError $ mkErrorNoPos $ ParseAmountErr $ T.unpack x
