...
--         Just template -> pure $ Just t1 -- fromTemplate vars template t

--   where applyMatch :: TransactionMatch -> Transaction -> Either (Maybe Error) (Transaction, M.Map String String)
--         applyMatch m t1 =  do
--            v1 <- matchDate (tDate m) (tDate t1)
--            (t2, v2) <- traverse $ matchPostings (map snd $ tPostings m) (tPostings t1)
--            (t3, v3) <- traverse $ matchTags (map snd $ tTags m) (tTags t1)
--            let vars = v1 ++ v2 ++ v3
